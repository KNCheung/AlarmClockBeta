...
typedef   CPU_INT08U      OS_CPU_USAGE;                /* CPU使用率 CPU Usage 0..100%                       <8>/16/32 */

typedef   CPU_INT32U      OS_CTR;                      /* 计数器Counter,                                           32 */

typedef   CPU_INT32U      OS_CTX_SW_CTR;               /* 上下文切换的计数器 Counter of context switches,          32 */

typedef   CPU_INT64U      OS_CYCLES;                   /* CPU时钟周期 CPU clock cycles,                       32/<64> */

typedef   CPU_INT32U      OS_FLAGS;                    /* 事件标志 Event flags,                             8/16/<32> */

typedef   CPU_INT32U      OS_IDLE_CTR;                 /* 空闲任务计数器 Holds the number of times the idle task runs,       <32>/64 */

typedef   CPU_INT16U      OS_MEM_QTY;                  /* 内存控制块的数量 Number of memory blocks,           <16>/32 */
typedef   CPU_INT16U      OS_MEM_SIZE;                 /* 一个内存快的大小 Size in bytes of a memory block,   <16>/32 */
...
typedef   CPU_INT16U      OS_OBJ_QTY;                  /* 内核对象计数器 Number of kernel objects counter,    <16>/32 */
typedef   CPU_INT32U      OS_OBJ_TYPE;                 /* Special flag to determine object type,                   32 */

typedef   CPU_INT16U      OS_OPT;                      /* 保存函数选项 Holds function options                 <16>/32 */

typedef   CPU_INT08U      OS_PRIO;                     /* 任务优先级 Priority of a task,                    <8>/16/32 */

typedef   CPU_INT16U      OS_QTY;                      /* Quantity                                            <16>/32 */

typedef   CPU_INT32U      OS_RATE_HZ;                  /* Rate in Hertz                                            32 */

typedef   CPU_INT32U      OS_REG;                      /* 任务寄存器 Task register                          8/16/<32> */
typedef   CPU_INT08U      OS_REG_ID;                   /* Index to task register                            <8>/16/32 */

typedef   CPU_INT32U      OS_SEM_CTR;                  /* 信号量数目 Semaphore                                16/<32> */

typedef   CPU_INT08U      OS_STATE;                    /* 状态变量 State variable                           <8>/16/32 */
