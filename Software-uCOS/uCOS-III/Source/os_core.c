...
void  OSInit (OS_ERR  *p_err)
{
    ...
    OSInitHook();                                           /* 呼叫初始化钩子函数 Call port specific initialization code                 */

    OSIntNestingCtr                 = (OS_NESTING_CTR)0;    /* 清零中断潜逃计数器 Clear the interrupt nesting counter                    */

    OSRunning                       =  OS_STATE_OS_STOPPED; /* 标明多任务未开始 Indicate that multitasking not started                 */

    OSSchedLockNestingCtr           = (OS_NESTING_CTR)0;    /* 清零调度锁寄存器 Clear the scheduling lock counter                      */
    ...
    OS_PrioInit();                                          /* 初始化优先级表 Initialize the priority bitmap table                   */

    OS_RdyListInit();                                       /* 初始化就绪表 Initialize the Ready List                              */

    OS_TaskInit(p_err);                                     /* 初始化任务管理 Initialize the task manager                            */
    if (*p_err != OS_ERR_NONE) {
        return;
    }

#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
    OS_IntQTaskInit(p_err);                                 /* 初始化中断队列处理任务 Initialize the Interrupt Queue Handler Task            */
    if (*p_err != OS_ERR_NONE) {
        return;
    }
#endif

    OS_IdleTaskInit(p_err);                                 /* 初始化空闲任务 Initialize the Idle Task                               */
    if (*p_err != OS_ERR_NONE) {
        return;
    }

    OS_TickTaskInit(p_err);                                 /* 初始化滴答任务 Initialize the Tick Task                               */
    if (*p_err != OS_ERR_NONE) {
        return;
    }
