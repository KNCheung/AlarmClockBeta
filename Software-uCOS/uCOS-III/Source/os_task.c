...
*
* Arguments  : p_tcb          任务控制块指针 is a pointer to the task's TCB
*
*              p_name         指向一个任务名字符串 is a pointer to an ASCII string to provide a name to the task.
*
*              p_task         指向任务名is a pointer to the task's code
*
...
*                                 }
*
*              prio           任务的优先级 is the task's priority.  A unique priority MUST be assigned to each task and the
*                             lower the number, the higher the priority.
*
*              p_stk_base     指向堆栈基址 is a pointer to the base address of the stack (i.e. low address).
*
...
*
*              q_size         能送到任务的消息最大数 is the maximum number of messages that can be sent to the task
*
                    ...
                    OS_ERR        *p_err)
{
    ...
    if (p_task == (OS_TASK_PTR)0) {                         /* 用户必须提供一个有效任务 User must supply a valid task                          */
        *p_err = OS_ERR_TASK_INVALID;
        return;
    }
    if (p_stk_base == (CPU_STK *)0) {                       /* 用户必须提供一个有效堆基地通讯地址 User must supply a valid stack base address            */
        *p_err = OS_ERR_STK_INVALID;
        return;
    }
    if (stk_size < OSCfg_StkSizeMin) {                      /* 用户必须提供一个有效最小的堆栈大小 User must supply a valid minimum stack size            */
        *p_err = OS_ERR_STK_SIZE_INVALID;
        return;
    }
    if (stk_limit >= stk_size) {                            /* 用户必须提供一个有效堆限制 User must supply a valid stack limit                   */
...
void  OS_TaskInit (OS_ERR  *p_err)
{
    ...
    OSTaskQty        = (OS_OBJ_QTY   )0;                    /* 清零任务数 Clear the number of tasks                              */
    OSTaskCtxSwCtr   = (OS_CTX_SW_CTR)0;                    /* 清零上下文计数器 Clear the context switch counter                       */
